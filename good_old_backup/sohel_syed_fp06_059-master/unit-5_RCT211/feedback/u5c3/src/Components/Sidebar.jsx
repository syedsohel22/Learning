...
export const Sidebar = () => {
  ...
  const initalState = searchParams.getAll("batch");
  const initalOrder = searchParams.get("order");
  const initalPage = searchParams.get("page");
  const [batch, setBatch] = useState(initalState || []);
  const [order, setOrder] = useState(initalOrder || []);
  const [page, setPage] = useState(+initalPage || 1);
  ...
  return (
    <DIV>
      ...
      <h3>Paginate</h3>
      <PAGE>
        <button data-testid="page-prev" onClick={() => handlePage(-1)}>
          Previous
        </button>
        <button data-testid="page-next" onClick={() => handlePage(1)}>
          Next
        </button>
      </PAGE>
    </DIV>
